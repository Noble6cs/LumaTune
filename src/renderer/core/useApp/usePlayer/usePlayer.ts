...
import { initRecentPlayListRecords, recordRecentPlayList } from '@renderer/store/player/recentPlay'
...
export default () => {
  ...
  const handleUpdatePlayInfo = () => {
    ...
    if (playMusicInfo.listId && playMusicInfo.musicInfo) {
      void recordRecentPlayList(playMusicInfo.listId, playMusicInfo.musicInfo.id)
    }
  }
