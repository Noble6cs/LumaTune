...
const getVisualWeight = (index, total) => {
  const ratio = index / total
  return 0.72 + Math.pow(ratio, 0.72) * 0.46
}
const getSmoothedSpectrum = (dataArray, index) => {
  const current = dataArray[index]
  const previous = dataArray[index - 1] ?? current
  const next = dataArray[index + 1] ?? current
  const previous2 = dataArray[index - 2] ?? previous
  const next2 = dataArray[index + 2] ?? next
  return current * 0.52 + previous * 0.18 + next * 0.18 + previous2 * 0.06 + next2 * 0.06
}
const compressSpectrum = value => Math.pow(Math.min(value, 255) / 255, 0.82) * 255
  ...
  setup() {
    ...
    const renderFrame = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        frequencyAvg += getSmoothedSpectrum(dataArray, i)
      }
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        barHeight = getSmoothedSpectrum(dataArray, i)
        barHeight = compressSpectrum(barHeight * getVisualWeight(i, bufferLength))
