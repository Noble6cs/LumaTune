    ...
    <div ref="dom_menu" :class="$style.container" :style="menuStyles" :aria-hidden="!modelValue">
      ...
      <div :class="$style.group">
        <div :class="$style.subGroup">
          ...
          <button :class="[$style.btn, $style.titleBtn]" :disabled="appSetting['playDetail.style.fontSize'] == 85" ignore-tip :aria-label="$t('lyric_menu__size_reset')" @click="fontSizeReset">{{ $t('lyric_menu__size_reset') }}</button>
        </div>
        <div :class="$style.subGroup">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fontSizeUp = step => {
      if (appSetting['playDetail.style.fontSize'] >= FONT_SIZE_MAX) return
      setPlayDetailLyricFont(Math.min(appSetting['playDetail.style.fontSize'] + step, FONT_SIZE_MAX))
    }
    const fontSizeDown = step => {
      if (appSetting['playDetail.style.fontSize'] <= FONT_SIZE_MIN) return
      setPlayDetailLyricFont(Math.max(appSetting['playDetail.style.fontSize'] - step, FONT_SIZE_MIN))
    }
    const fontSizeReset = () => {
      setPlayDetailLyricFont(FONT_SIZE_DEFAULT)
    }
