<template>
  <div ref="dom_lists" :class="$style.lists">
    ...
    <ul ref="dom_lists_list" class="scroll" :class="[$style.listsContent, { [$style.sortable]: isModDown }]">
      <li
        class="default-list" :class="[$style.listsItem, {[$style.active]: allMusicList.id == listId}]"
        :aria-label="$t(allMusicList.name)" :aria-selected="allMusicList.id == listId"
        @click="handleListToggle(allMusicList.id)"
      >
        <span :class="$style.listsLabel">
          <transition name="list-active">
            <svg-icon v-if="allMusicList.id == listId" name="angle-right-solid" :class="$style.activeIcon" />
          </transition>
          {{ $t(allMusicList.name) }}
        </span>
      </li>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(() => userLists, (lists) => {
      if (props.listId == LIST_IDS.ALL) return
